...
4. Configure the production Worker routes to the host's exact `/scorecard` path and `/scorecard/*` subtree. This Worker does not proxy or replace the organization's other website routes. Asset files live under `/scorecard/assets`. Test `/scorecard`, all detail links, archival links, and unrelated website routes on staging before activating the route.
...
Large drafts and publications use a versioned gzip/base64 storage envelope. Expanded bytes have a checksum and a declared size checked during bounded decompression. Legacy inline JSON remains readable; public API responses remain full ordinary JSON. The storage envelope preserves small metadata fields so publication lists and draft lists can query D1 without inflating every payload. The atomic publishing transaction and immutable triggers remain unchanged.

Copied envelope metadata must exactly match the decoded, checksummed payload; unknown or missing required metadata is refused. Drafts can be staged within the upload/storage limits while their complete frozen-score publication is too large. Review performs a full publication-capacity preflight, including a conservative publisher-identity and metadata reserve, before recording approval. An over-capacity draft remains unreviewed with a precise 413 response so staff can reduce its scope. Public corrections and staff control requests have a smaller 20,000-byte body limit; only dataset uploads use the 12 MB limit.

Requests are capped at **12,000,000 UTF-8 bytes**, expanded payloads at **32,000,000 bytes**, compressed gzip at **1,340,000 bytes**, and final encoded D1 payloads at **1,800,000 bytes**. Oversized or poorly compressing data returns 413 before insertion; evidence is never truncated. The former raw 1.8 MB publication cap has been replaced by this compressed-row cap. `npm run test:capacity` qualifies 535 fictional legislators through 250 chamber-specific rollcalls against the actual local workerd handler and D1. See [CAPACITY.md](CAPACITY.md) for measurements, environment limits and required production qualification; these local results do not establish edge memory headroom, concurrency or unlimited history capacity.
...
| GET `/staff/drafts`              | Latest 100 lightweight draft summaries, without Dataset payloads                          |
| GET `/staff/drafts/:id`          | One complete draft with decoded Dataset                                                   |
